...
test('DEFAULT_RUNTIME_WHEEL: 릴리스된 runtime(host 포함) wheel v3.8.7 을 가리킨다', () => {
  assert.equal(RUNTIME_WHEEL_VERSION, '3.8.7');
  assert.equal(
    DEFAULT_RUNTIME_WHEEL,
    'https://github.com/PlateerLab/xgen-agent-runtime/releases/download/v3.8.7/xgen_agent_runtime-3.8.7-py3-none-any.whl',
  );
  assert.match(DEFAULT_RUNTIME_WHEEL, /xgen-agent-runtime\/releases\/download\/v3\.8\.7\//);
