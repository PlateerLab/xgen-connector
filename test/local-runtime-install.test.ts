...
test('DEFAULT_RUNTIME_WHEEL: 릴리스된 runtime(host 포함) wheel v3.8.1 을 가리킨다', () => {
  assert.equal(RUNTIME_WHEEL_VERSION, '3.8.1');
  assert.equal(
    DEFAULT_RUNTIME_WHEEL,
    'https://github.com/PlateerLab/xgen-agent-runtime/releases/download/v3.8.1/xgen_agent_runtime-3.8.1-py3-none-any.whl',
  );
  assert.match(DEFAULT_RUNTIME_WHEEL, /xgen-agent-runtime\/releases\/download\/v3\.8\.1\//);
...
test('readInstalledVersion: dist-info 우선, 없으면 RUNTIME_VERSION 스탬프 (감사 #16)', () => {
  ...
  try {
    ...
    mkRuntime(dir, '3.8.1');
    assert.equal(readDistInfoVersion(dir), '3.8.1');
    assert.equal(readInstalledVersion(dir), '3.8.1');
  } finally {
...
test('writeRuntimeVersionStamp: 첫 줄만 바꾸고 python 태그 줄은 보존, 없으면 생성', () => {
  ...
  try {
    mkRuntime(dir);
    writeFileSync(runtimeVersionStampPath(dir), '3.7.0\n3.12.11+20250808\n');
    assert.equal(writeRuntimeVersionStamp(dir, '3.8.1'), true);
    assert.equal(readFileSync(runtimeVersionStampPath(dir), 'utf-8'), '3.8.1\n3.12.11+20250808\n');
    rmSync(runtimeVersionStampPath(dir));
    assert.equal(writeRuntimeVersionStamp(dir, '3.8.1'), true);
    assert.equal(readFileSync(runtimeVersionStampPath(dir), 'utf-8'), '3.8.1\n');
    assert.equal(readStampVersion(dir), '3.8.1');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test(
  ...
  async () => {
    ...
    try {
      ...
      writeFileSync(runtimeVersionStampPath(dir), '3.7.0\n3.12.11+20250808\n');
      // 가짜 python: `-m pip install …` 이면 dist-info 를 3.7.0 → 3.8.1 으로 바꾼다(pip 흉내), 그 외 0.
...
if [ "$1" = "-m" ] && [ "$2" = "pip" ]; then
  ...
  mkdir -p "${join(sp, 'xgen_agent_runtime-3.8.1.dist-info')}"
  printf 'Version: 3.8.1\\n' > "${join(sp, 'xgen_agent_runtime-3.8.1.dist-info', 'METADATA')}"
fi
exit 0
`,
      );
