...
test('bundle script 의 RELEASED_RUNTIME_VERSION 은 앱의 RUNTIME_WHEEL_VERSION 과 같다(3.14.0)', () => {
  ...
  assert.equal(m![1], '3.14.0');
  assert.match(
    src,
    /releases\/download\/v\$\{RELEASED_RUNTIME_VERSION\}\/xgen_agent_runtime-\$\{RELEASED_RUNTIME_VERSION\}-py3-none-any\.whl/,
  );
  assert.match(DEFAULT_RUNTIME_WHEEL, /\/v3\.14\.0\/xgen_agent_runtime-3\.14\.0-py3-none-any\.whl$/);
});
