...
test('드라이브는 기본으로 켜져 있고, 명시적으로만 꺼진다', () => {
...
test('현재 루트의 하위는 새 루트로 고를 수 없다 (마운트 안 마운트)', () => {
...
test('잔여 파일은 지우지 않고 옆으로 옮긴다 (사용자 파일이다)', () => {
  ...
  try {
    const backup = rescueStrays(mp, '20260805-1200')
...
test('비어 있으면 보관 폴더를 만들지 않는다', () => {
  const tmp = mkdtempSync(join(tmpdir(), 'xgen-stray-'))
  const mp = join(tmp, 'XGEN-Workspace')
  mkdirSync(mp)
  try {
    assert.equal(rescueStrays(mp, '20260805-1200'), null)
